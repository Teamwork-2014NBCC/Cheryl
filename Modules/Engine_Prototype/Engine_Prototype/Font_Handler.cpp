...
FuckFont::FuckFont( std::string TextureFileName, std::string widths_file, float fontsize, GLSLProgram *shader )
{
	//load the texture via the texture manager
	texID = get_game_engine().txtr_mgr.GetID( TextureFileName );
	if ( texID == 0 )
	{
...
void FuckFont::DrawTxt( bool whichFont, float x, float y, std::string output )
{
	dest_x = x;
	dest_y = y;
	...
	prog->setUniform( "in_Scale", 1.f ); //default scaling
	int letter;
...
float FuckFont::WidthText( bool whichFont, std::string output )
{
	int letter;
	float width_text = 0;
...
void Font_Handler::ChangeFontSize( float newSize )
{
	if ( newSize != fontSize )
	{
		...
		//load a font set
		ffont = new FuckFont( "font.png", "fontMetrics.dat", newSize, get_blit3d()->shader2d );
...
* Modified by:	Mark Murphy		Date: Nov. 25, 2014
*****************************************************/
Font_Handler::Font_Handler( float fSize )
{
	float fontSize = fSize;

	//load a font set
	ffont = new FuckFont( "font.png", "fontMetrics.dat", fSize, get_blit3d()->shader2d );
