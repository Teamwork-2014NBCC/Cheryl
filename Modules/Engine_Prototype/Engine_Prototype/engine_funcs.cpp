...
inline void game::Init()
{
	...
	//load a font set
	this->font = new Font_Handler( 48.0f );
...
inline void game::Draw( void )
{
	...
	get_blit3d()->SetMode( Blit3DRenderMode::BLIT3D, prog );
	...
	if ( font )
	{
		...
		font->Write( false, 22, 45, "Hello World" );
		font->Write( false, 100, 100, "What!?" );
	}

	prog = get_blit3d()->sManager->UseShader( "shader.vert", "shader.frag" );
	get_blit3d()->SetMode( Blit3DRenderMode::BLIT3D, prog );
