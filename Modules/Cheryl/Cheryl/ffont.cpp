...
FFont::FFont( std::string TextureFileName, std::string widths_file, float fontsize, GLSLProgram *shader )
{
...
void FFont::DrawTxt( bool whichFont, float x, float y, std::string output )
{
	dest_x = x;
	dest_y = y;
...
float FFont::WidthText( bool whichFont, std::string output )
{
	int letter;
	float width_text = 0;
...
Font_Handler::Font_Handler( float fSize )
{
	...
	//load a font set
	ffont = new FFont( "font.png", "fontMetrics.dat", fSize, get_blit3d()->shader2d );
